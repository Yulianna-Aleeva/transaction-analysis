...
class TestGetStockPrices:
    ...
    def test_settings_present(
        ...
        assert result == [
            {"stock": "AAPL", "price": 195.25},
            {"stock": "SBER", "price": 310.4},
        ]
    ...
    def test_missing_trading_url(self) -> None:
        ...
        with patch(f"{MODULE_PATH}.USER_SETTINGS", settings):
            with pytest.raises(ValueError, match="Отсутствуют настройки: trading_url или user_stocks"):
                get_stock_prices()
    ...
    def test_both_sources_failed(
        ...
        with patch(f"{MODULE_PATH}.USER_SETTINGS", default_settings):
            with pytest.raises(ValueError, match="Не удалось получить данные ни с MOEX, ни с Yahoo"):
                get_stock_prices()
    ...
    def test_rounding(
        ...
        assert result == [{"stock": "AAPL", "price": 195.26}]
    ...
    def test_invalid_moex_price(
        ...
        """Игнорирует тикер (пустой список) при некорректной цене MOEX."""
        settings = {
            "trading_url": "https://moex.test",
            "user_stocks": ["AAPL"],
        }
        ...
        assert result == []
        mock_logger.debug.assert_any_call("Некорректная цена для %s: %s", "AAPL", "N/A")
    ...
    def test_logging(
        ...
        try:
            with (
                ...
                patch(f"{MODULE_PATH}.requests.get", return_value=moex_response),
    ...
    def test_moex_dict_format(self, default_settings: Settings) -> None:
    ...
    def test_moex_unknown_error_yahoo_success(self) -> None:
        ...
        assert result == [{"stock": "AAPL", "price": 195.25}]
    ...
    def test_yahoo_series_close(self) -> None:
        """Обрабатывает Yahoo Close как Series для одного тикера."""
        settings = {"trading_url": "https://moex.test", "user_stocks": ["AAPL"]}
